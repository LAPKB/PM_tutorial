...
-   Use the new Pmetrics Papir app, coming soon. The Shiny app is retired.
...
# example only; do not execute
sec = function() { # OR eqn = function() { OR lag = function() { OR fa = function() { OR ini = function() { OR out = function() {
...
You can access the Pmetrics Model Library programmatically using the `r gh_help("model_lib")` function or `r gh_help("apps")` for a listing of all available Pmetrics apps. The `r gh_help("model_lib")` function returns a data frame of available models with their characteristics, including the model name to be used in the `EQN` block, number of compartments, number of inputs and outputs, and a brief description.
...
The error list element is a list of creator functions. The two possible creator functions are `proportional` or `additive`. Each of these functions has the same arguments: `initial`, `coeff`, `fixed`, and `outeq`.
...
* The `fixed` argument is `FALSE` by default, which means that $\gamma$ or $\lambda$ will be estimated for each output equation as measures of additional noise in the data and/or model misspecification. If `fixed` is set to `TRUE`, then the value in `initial` will be held constant during estimation.
* `outeq` specifies which output equation the error equation belongs to. The default is `outeq = 1`, so if you only have one output equation (e.g. only `Y[1]` in your `out` block), you don't need to supply `outeq` here. If you have more than one output, you will need to specify that here.
...
mod <- PM_model$new(
  ...
  err = list(
    ...
    additive(0.1, c(0.1, 0, 0, 0), fixed = TRUE, outeq = 2)
  )
)
```
:::
...
To update a model, simply copy the text from the model list in R, and modify the relevant fields. Then reassign the modified model to a new object.

```{r}
#| label:  mod-block-update-example
# these examples will compile and can be executed 
...
mod2 <- PM_model$new(
  pri = list(
    Ka = ab(0.1, 0.9),
    Ke = ab(0.001, 0.5), # changed range
    K23 = ab(0, 5),
    K32 = ab(0, 5),
    V0 = ab(30, 120), # note the new name
    lag1 = ab(0, 4)
  ),
  cov = list(
    wt = interp(),
  ...
  sec = function() { # new secondary block
    V = V0 * wt # the library model expects a parameter named "V" defined somewhere
  },
  eqn = function() {
    two_comp_bolus
  },
...
This example changes the range of *Ke*, deletes *V*, adds *V0*, keeps *K23* and *K32* for `two_comp_bolus`, and defines a new secondary relationship for *V = V0 \* wt*.

### File

The only way to update a model file is to edit the text file and load it with `PM_model$new()`. The disadvantage is that you have to keep editing, copying and pasting files, and unless your documentation in R or elsewhere is good, you have no idea how you changed the model from run to run. In contrast, by using the direct model definition in R, you can see a written record in your script of how the model evolved over the project.
