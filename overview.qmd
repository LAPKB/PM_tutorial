...
**As of v. 3.2**, `r pmetrics()` uses a just in time (JIT) Rust compiler and direct symbolic language (DSL) under the hood, which means that users can enjoy the speed of compiled models without having to install Rust on their systems. This is a major breakthrough not available in most other pharmacometric software tools, which typically require complex installations of Fortran or other backend languages.
