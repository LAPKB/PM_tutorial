...
Once a `PM_model` object is compiled, combine it with a data file, and fit the model to the data using the `$fit()` method as we will see in more detail in subsequent chapters.
...
# example; will not run because mod1 and data are not yet defined
...
# examples; will not run because mod1 and data are not yet defined
run1 <- PM_load(1)
